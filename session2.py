...
class Something(object):
    ...
    def __repr__(self):
        return 'Something(None)' if self.something_new == None else f'Something(SomethingNew({self.something_new.i}, Something))'
...
class SomethingNew(object):
    ...
    def __repr__(self):
        return f'SomethingNew({self.i}, Something)'
...
def clear_memory(collection: List[Something]):
    # you probably need to add some content here
#    a_id = id(collection[1])
#    b_id = id(collection[1].something_new)
    collection.clear()
    gc.collect()
#    return (ref_count(a_id), ref_count(b_id))
...
def compare_strings_new(n):
    ...
    for i in range(n):
        if a is b:
            pass
    char_collection = set(a)
    for i in range(n):
        if 'd' in char_collection:
            pass
